...
class TrackSelectionInvestigationTest {
        ...
        for ((decoderName, profileLevels) in testDecoderCapabilities) {
            ...
            for (format in formats) {
                ...
                try {
                    ...
                    val isSupported = codecInfo.isFormatSupported(testContext, format)
                    val isFunctionallySupported = codecInfo.isFormatFunctionallySupported(testContext, format)
    ...
    fun testBuildStreamInfoDataWithActiveVideoFormat() {
        ...
        every { controller.currentInternalPlayerEngine } returns com.sluggyard.tv.data.local.InternalPlayerEngine.EXOPLAYER
    ...
    fun testDolbyVisionExtractorsFactoryFormatRewriting() {
        ...
        val factory = com.sluggyard.tv.core.player.DolbyVisionExtractorsFactory(
            delegate = delegateFactory,
            config = com.sluggyard.tv.core.player.DolbyVisionConversionConfig(active = false),
            stripDvRpu = true
        )
