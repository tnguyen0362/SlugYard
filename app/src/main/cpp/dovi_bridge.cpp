...
Java_com_sluggyard_tv_core_player_DoviBridge_nativeGetBridgeVersion(JNIEnv* env, jclass /* clazz */) {
#if DOVI_REAL_LINKED
...
Java_com_sluggyard_tv_core_player_DoviBridge_nativeIsConversionPathReady(
    JNIEnv* /* env */,
    jclass /* clazz */
) {
#if DOVI_REAL_LINKED
...
Java_com_sluggyard_tv_core_player_DoviBridge_nativeConvertDv7RpuToDv81(
    JNIEnv* env,
    jclass /* clazz */,
    jbyteArray payload,
    jint mode
...
Java_com_sluggyard_tv_core_player_DoviBridge_nativeConvertDv7RpuToDv81NonAllocating(
    JNIEnv* env,
    jclass /* clazz */,
    jbyteArray sample,
    jint offset,
...
extern "C" JNIEXPORT jint JNICALL
Java_com_sluggyard_tv_core_player_DoviBridge_nativeProcessVideoSample(
    JNIEnv* env,
    jclass /* clazz */,
    jbyteArray sample,
    jint sampleLen,
    ...
    if (nalFormat == 1) { // Length-Delimited
        ...
        while (pos + nalLengthFieldLength <= sampleLen) {
            ...
            if (convertDovi || stripDoviRpu) {
                ...
                // Type 63 carries the P7 enhancement layer in single-track
                // remuxes even when the layer id is zero.
