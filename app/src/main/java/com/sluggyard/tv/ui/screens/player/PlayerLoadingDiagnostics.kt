...
import com.sluggyard.tv.data.repository.PlaybackIssueLoadingEventInput
import com.sluggyard.tv.data.repository.PlaybackIssueLoadingInput
...
/**
 * Diagnostic tracking for the player startup/loading sequence.
 *
 * These extension functions on [PlayerRuntimeController] record phase
 * transitions, deduplicate identical consecutive updates, persist a raw event
 * line for issue reports, and surface a "Report playback issue" affordance
 * after a long stall.
 */
...
internal fun PlayerRuntimeController.recordLoadingDiagnosticEvent(
    ...
    while (loadingDiagnosticEvents.size > LOADING_EVENT_LIMIT) loadingDiagnosticEvents.removeFirst()
}
...
internal fun PlayerRuntimeController.finishLoadingDiagnostics(phase: String) {
    ...
    _uiState.update { it.copy(loadingIssueReportVisible = false, loadingIssueElapsedMs = 0L) }
}
...
internal fun PlayerRuntimeController.scheduleLoadingIssueReportAvailability() {
    ...
    startupLoadingReportJob = scope.launch {
        ...
        if (state.playbackIssueReportsEnabled && !hasRenderedFirstFrame && state.error == null && state.showLoadingOverlay) {
            ...
            _uiState.update { it.copy(loadingIssueReportVisible = true, loadingIssueElapsedMs = elapsedMs) }
        }
    }
}
...
private fun String.safePlaybackRawHost(): String = runCatching {
