...
import com.sluggyard.tv.data.repository.PlaybackIssuePlaybackAnalyticsInput
import com.sluggyard.tv.data.repository.PlaybackIssuePlaybackEventInput
import com.sluggyard.tv.data.repository.PlaybackIssuePlaybackFormatInput
import com.sluggyard.tv.data.repository.PlaybackIssuePlaybackHealthSnapshotInput
import com.sluggyard.tv.data.repository.PlaybackIssuePlaybackLoadErrorInput
import com.sluggyard.tv.data.repository.PlaybackIssuePlaybackLoadInput
...
/**
 * Collects ExoPlayer analytics + diagnostics into a structured snapshot for the
 * "Report playback issue" flow.
 *
 * Tracks playback state, decoder init/release, video/audio format changes,
 * dropped frames, audio underruns, bandwidth estimates, load events/errors,
 * position stalls, rebuffers, and periodic health snapshots — all bounded by
 * ring-buffer limits to keep memory stable over long sessions.
 */
internal class PlayerPlaybackAnalyticsDiagnostics {
    ...
    fun reset() {
        ...
        eventCount = 0
        playbackState = null; playbackStateName = null
        playWhenReady = null; isPlaying = null; isLoading = null
        playbackSpeed = null; playbackPitch = null
        positionMs = null; bufferedPositionMs = null; durationMs = null; bufferedPercentage = null
        firstFrameElapsedMs = null; renderedFirstFrameCount = 0
        lastPositionForStallMs = -1L
        positionLastAdvancedAtMs = sessionStartedAtElapsedMs
        positionStallActive = false; positionStallCount = 0; longestPositionStallMs = 0L
        droppedFrames = 0; maxDroppedFramesInEvent = 0
        videoDecoderName = null; videoDecoderInitMs = null; videoDecoderReleaseCount = 0
        videoRenderedOutputBuffers = null; videoDroppedBuffers = null; videoMaxConsecutiveDroppedBuffers = null
        videoFrameProcessingOffsetTotalUs = 0L; videoFrameProcessingOffsetCount = 0; videoFormat = null
        audioDecoderName = null; audioDecoderInitMs = null; audioDecoderReleaseCount = 0
        audioUnderrunCount = 0; audioUnderrunBufferSize = null
        audioUnderrunBufferSizeMs = null; audioUnderrunElapsedSinceLastFeedMs = null; audioFormat = null
        bandwidthEstimateBps = null; bandwidthTransferDurationMs = null; bandwidthBytesTransferred = null
        loadStartedCount = 0; loadCompletedCount = 0; loadCanceledCount = 0; loadErrorCount = 0
        totalBytesLoaded = 0L; lastLoad = null; lastLoadError = null
        rawEventLines.clear()
        launchStartedAtElapsedMs = null; initializationStartedAtWallTimeMs = 0L
        startPositionMs = null; lastHealthSnapshotAtElapsedMs = 0L
    }

    fun recordRawEventLine(line: String) {
        rawEventLines.addLast(line.rawPlaybackLine())
        while (rawEventLines.size > PLAYBACK_RAW_EVENT_LIMIT) rawEventLines.removeFirst()
    }
    ...
    fun recordProgressSnapshot(
        ...
        recordHealthSnapshotIfNeeded(now, rebufferCount, rebufferTotalMs, force = false)

        val shouldDetectStall = hasRenderedFirstFrame && player.playWhenReady && player.isPlaying &&
    ...
    fun onPlaybackStateChanged(eventTime: AnalyticsListener.EventTime, state: Int) {
        ...
        record(name = "playback_state", eventTime = eventTime, details = details("state" to state.playbackStateName()))
    }

    fun onPlayWhenReadyChanged(eventTime: AnalyticsListener.EventTime, ready: Boolean, reason: Int) {
        playWhenReady = ready
        record(name = "play_when_ready", eventTime = eventTime, details = details("ready" to ready.toString(), "reason" to reason.toString()))
    }

    fun onIsPlayingChanged(eventTime: AnalyticsListener.EventTime, playing: Boolean) {
        isPlaying = playing
        record(name = "is_playing", eventTime = eventTime, details = details("playing" to playing.toString()))
    }

    fun onIsLoadingChanged(eventTime: AnalyticsListener.EventTime, loading: Boolean) {
        isLoading = loading
        record(name = "is_loading", eventTime = eventTime, details = details("loading" to loading.toString()))
    }

    fun onPlaybackParametersChanged(eventTime: AnalyticsListener.EventTime, playbackParameters: PlaybackParameters) {
        playbackSpeed = playbackParameters.speed
        playbackPitch = playbackParameters.pitch
        record(
            name = "playback_parameters", eventTime = eventTime,
            details = details("speed" to playbackParameters.speed.toString(), "pitch" to playbackParameters.pitch.toString())
        )
    }
    ...
    fun onSyntheticFirstFrame(player: Player?) {
        ...
        record(name = "first_frame_ready", eventTime = null, details = details("source" to "state_ready"))
    }

    fun onRebufferStarted(player: Player?, count: Int) {
        player?.let { recordProgressSnapshot(it, hasRenderedFirstFrame = true) }
        record(
            name = "rebuffer_start", eventTime = null,
            details = details(
    ...
    fun onRebufferEnded(player: Player?, totalMs: Long, lastDurationMs: Long) {
        player?.let { recordProgressSnapshot(it, hasRenderedFirstFrame = true) }
        record(
            name = "rebuffer_end", eventTime = null,
            details = details(
    ...
    fun onVideoDecoderInitialized(eventTime: AnalyticsListener.EventTime, decoderName: String, initializationDurationMs: Long) {
        videoDecoderName = decoderName
        videoDecoderInitMs = initializationDurationMs.coerceAtLeast(0L)
        record(name = "video_decoder_initialized", eventTime = eventTime, details = details("decoder" to decoderName, "initMs" to videoDecoderInitMs?.toString()))
    }

    fun onVideoDecoderReleased(eventTime: AnalyticsListener.EventTime, decoderName: String) {
        videoDecoderReleaseCount += 1
        record(name = "video_decoder_released", eventTime = eventTime, details = details("decoder" to decoderName, "count" to videoDecoderReleaseCount.toString()))
    }

    fun onVideoInputFormatChanged(eventTime: AnalyticsListener.EventTime, format: Format, reuseEvaluation: DecoderReuseEvaluation?) {
        ...
        record(name = "video_format", eventTime = eventTime, details = format.formatDetails(C.TRACK_TYPE_VIDEO.trackTypeName()))
    }
    ...
    fun onVideoTrackSnapshot(format: Format, support: String?, selected: Boolean) {
        ...
        record(
            name = "video_track_snapshot", eventTime = null,
            details = format.formatDetails(C.TRACK_TYPE_VIDEO.trackTypeName()) + details("support" to support, "selected" to selected.toString())
        )
    }

    fun onVideoSizeChanged(eventTime: AnalyticsListener.EventTime, videoSize: VideoSize) {
        record(
            name = "video_size", eventTime = eventTime,
            details = details(
    ...
    fun onDroppedVideoFrames(eventTime: AnalyticsListener.EventTime, count: Int, elapsedMs: Long) {
        ...
        record(
            name = "dropped_video_frames", eventTime = eventTime,
            details = details(
    ...
    fun onVideoFrameProcessingOffset(eventTime: AnalyticsListener.EventTime, totalProcessingOffsetUs: Long, frameCount: Int) {
        if (frameCount > 0) {
            videoFrameProcessingOffsetTotalUs += totalProcessingOffsetUs
            videoFrameProcessingOffsetCount += frameCount
        }
        record(
            name = "video_frame_processing_offset", eventTime = eventTime,
            details = details(
    ...
    fun onVideoDisabled(eventTime: AnalyticsListener.EventTime, counters: DecoderCounters) {
        ...
        record(
            name = "video_disabled", eventTime = eventTime,
            details = details(
    ...
    fun onAudioDecoderInitialized(eventTime: AnalyticsListener.EventTime, decoderName: String, initializationDurationMs: Long) {
        audioDecoderName = decoderName
        audioDecoderInitMs = initializationDurationMs.coerceAtLeast(0L)
        record(name = "audio_decoder_initialized", eventTime = eventTime, details = details("decoder" to decoderName, "initMs" to audioDecoderInitMs?.toString()))
    }

    fun onAudioDecoderReleased(eventTime: AnalyticsListener.EventTime, decoderName: String) {
        audioDecoderReleaseCount += 1
        record(name = "audio_decoder_released", eventTime = eventTime, details = details("decoder" to decoderName, "count" to audioDecoderReleaseCount.toString()))
    }

    fun onAudioInputFormatChanged(eventTime: AnalyticsListener.EventTime, format: Format, reuseEvaluation: DecoderReuseEvaluation?) {
        audioFormat = format.toPlaybackFormat(
            trackType = C.TRACK_TYPE_AUDIO.trackTypeName(),
            support = null,
            reuseEvaluation = reuseEvaluation
        )
        record(name = "audio_format", eventTime = eventTime, details = format.formatDetails(C.TRACK_TYPE_AUDIO.trackTypeName()))
    }

    fun onAudioUnderrun(eventTime: AnalyticsListener.EventTime, bufferSize: Int, bufferSizeMs: Long, elapsedSinceLastFeedMs: Long) {
        ...
        record(
            name = "audio_underrun", eventTime = eventTime,
            details = details(
    ...
    fun onBandwidthEstimate(eventTime: AnalyticsListener.EventTime, totalLoadTimeMs: Int, totalBytesLoaded: Long, bitrateEstimate: Long) {
        ...
        record(
            name = "bandwidth_estimate", eventTime = eventTime,
            details = details(
    ...
    fun onLoadStarted(eventTime: AnalyticsListener.EventTime, loadEventInfo: LoadEventInfo, mediaLoadData: MediaLoadData) {
        loadStartedCount += 1
        lastLoad = loadEventInfo.toPlaybackLoad(mediaLoadData)
        record(name = "load_started", eventTime = eventTime, details = loadDetails(lastLoad))
    }

    fun onLoadCompleted(eventTime: AnalyticsListener.EventTime, loadEventInfo: LoadEventInfo, mediaLoadData: MediaLoadData) {
        loadCompletedCount += 1
        totalBytesLoaded += loadEventInfo.bytesLoaded.coerceAtLeast(0L)
        lastLoad = loadEventInfo.toPlaybackLoad(mediaLoadData)
        record(name = "load_completed", eventTime = eventTime, details = loadDetails(lastLoad))
    }

    fun onLoadCanceled(eventTime: AnalyticsListener.EventTime, loadEventInfo: LoadEventInfo, mediaLoadData: MediaLoadData) {
        loadCanceledCount += 1
        lastLoad = loadEventInfo.toPlaybackLoad(mediaLoadData)
        record(name = "load_canceled", eventTime = eventTime, details = loadDetails(lastLoad))
    }
    ...
    fun onLoadError(
        ...
        lastLoadError = PlaybackIssuePlaybackLoadErrorInput(
            host = load.host, dataType = load.dataType, trackType = load.trackType,
            exceptionClass = error.javaClass.name, message = error.message,
            httpStatus = error.findHttpStatus(), wasCanceled = wasCanceled,
            bytesLoaded = load.bytesLoaded, durationMs = load.durationMs
        )
        record(
            name = "load_error", eventTime = eventTime,
    ...
    fun onPlayerError(eventTime: AnalyticsListener.EventTime, error: PlaybackException) {
        record(
            name = "player_error", eventTime = eventTime,
            details = details(
    ...
    fun snapshot(
        ...
        player?.let {
            recordProgressSnapshot(it, hasRenderedFirstFrame, rebufferCount, rebufferTotalMs)
        }
        val now = SystemClock.elapsedRealtime()
        val capturedAtMs = System.currentTimeMillis()
        val currentRebufferMs = if (rebufferStartedAtMs > 0L) (now - rebufferStartedAtMs).coerceAtLeast(0L) else 0L
        recordHealthSnapshotIfNeeded(now, rebufferCount, rebufferTotalMs, force = true)
    ...
    private fun recordHealthSnapshotIfNeeded(now: Long, rebufferCount: Int, rebufferTotalMs: Long, force: Boolean) {
        ...
        ExperimentalDiagnostics.event(
            ...
            mapOf(
                "host" to traceHost,
                "engine" to traceEngine,
                "elapsedMs" to (now - sessionStartedAtElapsedMs).coerceAtLeast(0L),
                "state" to playbackStateName,
                ...
                "audioUnderruns" to audioUnderrunCount,
                "rebuffers" to rebufferCount,
                "rebufferTotalMs" to rebufferTotalMs,
                "bandwidthBps" to bandwidthEstimateBps,
                "totalBytesLoaded" to totalBytesLoaded,
                "loadStarted" to loadStartedCount,
                "loadCompleted" to loadCompletedCount,
                "loadCanceled" to loadCanceledCount,
                "loadErrors" to loadErrorCount,
            ),
        )
        while (healthSnapshots.size > PLAYBACK_HEALTH_SNAPSHOT_LIMIT) healthSnapshots.removeFirst()
    }

    private fun videoFrameProcessingOffsetAverageUs(): Long? =
        if (videoFrameProcessingOffsetCount > 0) videoFrameProcessingOffsetTotalUs / videoFrameProcessingOffsetCount
        else null
    ...
    private fun record(
        ...
        recordRawEventLine(
            buildRawEventLine(name, (now - sessionStartedAtElapsedMs).coerceAtLeast(0L), position, bufferedPosition, details)
        )
        events.addLast(
        ...
        while (events.size > PLAYBACK_ANALYTICS_EVENT_LIMIT) events.removeFirst()
        ExperimentalDiagnostics.event(
            "playback",
            "exo_event",
            mapOf(
                ...
                "details" to details.toTraceDetails(),
            ),
        )
    }
...
private fun String.isPlaybackWarningEvent(): Boolean =
    this == "dropped_video_frames" || this == "audio_underrun" ||
        this == "position_stall" || this == "position_stall_recovered" ||
        this == "rebuffer_start" || this == "rebuffer_end" ||
        this == "load_error" || this == "player_error"

private fun Map<String, String>.toTraceDetails(): String =
    if (isEmpty()) "n/a"
    else entries.joinToString(",") { "${it.key}=${it.value.compactTraceValue()}" }

private fun String.compactTraceValue(): String = replace('\n', ' ').replace('\r', ' ').take(160)

private fun String.rawPlaybackLine(): String = replace('\n', ' ').replace('\r', ' ').take(2000)

private fun LoadEventInfo.toPlaybackLoad(mediaLoadData: MediaLoadData): PlaybackIssuePlaybackLoadInput =
    PlaybackIssuePlaybackLoadInput(
        host = uri.host,
        scheme = uri.scheme,
...
): PlaybackIssuePlaybackFormatInput = PlaybackIssuePlaybackFormatInput(
    trackType = trackType,
...
private fun Format.formatDetails(trackType: String?): Map<String, String> = details(
...
private fun loadDetails(load: PlaybackIssuePlaybackLoadInput?): Map<String, String> = details(
    "host" to load?.host,
...
private fun Long?.safeTimeMs(): Long? = this?.takeIf { it != C.TIME_UNSET && it >= 0L }
...
private fun Int.decoderReuseResultName(): String = when (this) {
