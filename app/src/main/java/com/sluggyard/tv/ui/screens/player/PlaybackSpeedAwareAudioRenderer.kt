...
/**
 * [MediaCodecAudioRenderer] that consults [PlaybackSpeedAwareAudioSink] before
 * deciding whether to bypass the codec for passthrough formats. When the sink
 * has locked PCM for the current session (because a non-unit speed was used),
 * the renderer refuses bypass and falls back to PCM decoding.
 */
internal class PlaybackSpeedAwareAudioRenderer(
    ...
    override fun supportsFormat(mediaCodecSelector: MediaCodecSelector, format: Format): Int {
        ...
        if (supportsFormatDrm && (!formatHasDrm || MediaCodecUtil.getDecryptOnlyDecoderInfo() != null)) {
            audioOffloadSupport = computeRendererAudioOffloadSupport(format, forcePcm)
        ...
        if (!isFormatSupported) {
            for (i in 1 until decoderInfos.size) {
                val other = decoderInfos[i]
                if (other.isFormatSupported(rendererContext, format)) {
                    decoderInfo = other
        ...
        val adaptiveSupport = if (isFormatSupported && decoderInfo.isSeamlessAdaptationSupported(format)) ADAPTIVE_SEAMLESS
        else ADAPTIVE_NOT_SEAMLESS
        val hardwareAccelerationSupport = if (decoderInfo.hardwareAccelerated) HARDWARE_ACCELERATION_SUPPORTED
        ...
        return RendererCapabilities.create(
            formatSupport, adaptiveSupport, TUNNELING_SUPPORTED,
            hardwareAccelerationSupport, decoderSupport, audioOffloadSupport
        )
    }
    ...
    override fun getDecoderInfos(
        ...
        val raw = if (!playbackSpeedAwareAudioSink.shouldForcePcmForFormat(format) && playbackSpeedAwareAudioSink.supportsFormat(format)) {
            MediaCodecUtil.getDecryptOnlyDecoderInfo()?.let(::listOf)
                ?: MediaCodecUtil.getDecoderInfosSoftMatch(mediaCodecSelector, format, requiresSecureDecoder, false)
        } else {
            MediaCodecUtil.getDecoderInfosSoftMatch(mediaCodecSelector, format, requiresSecureDecoder, false)
        }
        return MediaCodecUtil.getDecoderInfosSortedByFormatSupport(rendererContext, raw, format)
    }

    override fun shouldUseBypass(format: Format): Boolean {
        if (playbackSpeedAwareAudioSink.shouldForcePcmForFormat(format)) return false
        return super.shouldUseBypass(format)
    }

    private fun computeRendererAudioOffloadSupport(format: Format, forcePcm: Boolean): Int {
        if (forcePcm) return AUDIO_OFFLOAD_NOT_SUPPORTED
        val sinkOffload = playbackSpeedAwareAudioSink.getFormatOffloadSupport(format)
        if (!sinkOffload.isFormatSupported) return AUDIO_OFFLOAD_NOT_SUPPORTED
        var support = AUDIO_OFFLOAD_SUPPORTED
        if (sinkOffload.isGaplessSupported) support = support or AUDIO_OFFLOAD_GAPLESS_SUPPORTED
        if (sinkOffload.isSpeedChangeSupported) support = support or AUDIO_OFFLOAD_SPEED_CHANGE_SUPPORTED
        return support
    }
}