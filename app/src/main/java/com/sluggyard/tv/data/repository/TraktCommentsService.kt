...
import com.sluggyard.tv.domain.model.TraktCommentReview
...
/**
 * Fetches paginated Trakt comments/reviews for a movie, show, or episode,
 * resolving the canonical Trakt path id from the supplied [Meta] (with a TMDB
 * search fallback) and caching pages in memory for a short TTL.
 */
@Singleton
class TraktCommentsService @Inject constructor(
    ...
    suspend fun getCommentsPage(
        ...
        targetEpisode: com.sluggyard.tv.domain.model.Video? = null,
        page: Int = 1,
        ...
        } ?: throw IllegalStateException(appContext.getString(com.sluggyard.tv.R.string.trakt_comments_error_request_failed))
    ...
    private suspend fun resolveCommentsTarget(
        ...
        targetEpisode: com.sluggyard.tv.domain.model.Video?
    ): ResolvedCommentsTarget? {
        val type = resolveCommentsType(meta, fallbackItemType, targetEpisode) ?: return null
...
internal fun filterDisplayableComments(comments: List<TraktCommentDto>): List<TraktCommentDto> =
    comments.filter { !it.comment.isNullOrBlank() }
...
private fun toReviewModel(dto: TraktCommentDto, appContext: android.content.Context): TraktCommentReview {
    val authorDisplayName = dto.user?.name?.takeIf { it.isNotBlank() }
        ?: dto.user?.username?.takeIf { it.isNotBlank() }
        ?: appContext.getString(com.sluggyard.tv.R.string.trakt_user_fallback)
