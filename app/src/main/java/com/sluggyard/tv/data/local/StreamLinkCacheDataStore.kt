...
class StreamLinkCacheDataStore @Inject constructor(
    ...
    private companion object {
        const val FEATURE_NAME = "stream_link_cache"
        const val KEY_PREFIX = "stream_link_"
    }
    ...
    suspend fun getValid(contentKey: String, maxAgeMs: Long): CachedStreamLink? {
        ...
        if (parsed == null) {
            store().edit { mutablePrefs -> mutablePrefs.remove(key) }
        }

        return parsed
    }

    private fun hashedKey(contentKey: String): Preferences.Key<String> {
