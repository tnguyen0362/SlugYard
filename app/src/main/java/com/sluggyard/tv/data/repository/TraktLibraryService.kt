...
import com.sluggyard.tv.core.trakt.traktBestBackdropUrl
import com.sluggyard.tv.core.trakt.traktBestLogoUrl
...
import com.sluggyard.tv.data.remote.dto.trakt.TraktListMovieRequestItemDto
import com.sluggyard.tv.data.remote.dto.trakt.TraktListShowRequestItemDto
...
/**
 * Backs the Trakt library: watchlist + personal lists. Maintains an
 * in-memory snapshot of tabs/entries/membership, applies optimistic mutations
 * on add/remove, and refreshes the snapshot on profile switches.
 */
@Singleton
class TraktLibraryService @Inject constructor(
    ...
    fun observeListTabs(): Flow<List<LibraryListTab>> =
        snapshotState.map { it.listTabs }.distinctUntilChanged().onStart { ensureFresh() }

    fun observeAllItems(): Flow<List<LibraryEntry>> =
        snapshotState.map { it.allEntries }.distinctUntilChanged().onStart { ensureFresh() }

    fun observeMembership(itemId: String, itemType: String): Flow<Set<String>> {
        val key = contentKey(itemId = itemId, itemType = itemType)
        return snapshotState.map { it.membershipByContent[key].orEmpty() }.distinctUntilChanged().onStart { ensureFresh() }
    }
    ...
    suspend fun getMembershipSnapshot(item: LibraryEntryInput): ListMembershipSnapshot {
        ...
        val map = snapshot.listTabs.associate { tab -> tab.key to memberships.contains(tab.key) }
    ...
    suspend fun toggleWatchlist(item: LibraryEntryInput) {
        ...
        if (isInWatchlist) {
            performOptimisticMutation(optimistic = { removeItemFromList(it, item, WATCHLIST_KEY) }) {
                removeFromWatchlist(item)
            }
        } else {
            performOptimisticMutation(optimistic = { addItemToList(it, item, WATCHLIST_KEY) }) {
                addToWatchlist(item)
            }
        }
    }
    ...
    suspend fun applyMembershipChanges(item: LibraryEntryInput, changes: ListMembershipChanges) {
        ...
        keys.forEach { listKey ->
            ...
            if (listKey == WATCHLIST_KEY) {
                if (after) {
                    performOptimisticMutation(optimistic = { addItemToList(it, item, WATCHLIST_KEY) }) { addToWatchlist(item) }
                } else {
                    performOptimisticMutation(optimistic = { removeItemFromList(it, item, WATCHLIST_KEY) }) { removeFromWatchlist(item) }
                }
            } else {
                val listId = listIdFromKey(listKey) ?: return@forEach
                if (after) {
                    performOptimisticMutation(optimistic = { addItemToList(it, item, listKey) }) { addToPersonalList(listId, item) }
                } else {
                    performOptimisticMutation(optimistic = { removeItemFromList(it, item, listKey) }) { removeFromPersonalList(listId, item) }
                }
            }
        }
    }

    suspend fun createPersonalList(name: String, description: String?, privacy: TraktListPrivacy) {
        val response = traktAuthService.executeAuthorizedRequest { authHeader ->
            traktApi.createUserList(
                authorization = authHeader,
                id = ME_PATH,
                body = TraktCreateOrUpdateListRequestDto(name = name, description = description, privacy = privacy.apiValue)
            )
        } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_request_failed))

        if (!response.isSuccessful) {
            throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_create_list_failed)))
        }
        ...
        val updatedEntries = if (createdTab.key in existingTabKeys) current.entriesByList
        else current.entriesByList + (createdTab.key to emptyList())
        snapshotState.value = rebuildSnapshot(updatedTabs, updatedEntries)
    }

    suspend fun updatePersonalList(listId: String, name: String, description: String?, privacy: TraktListPrivacy) {
        performOptimisticMutation(optimistic = { snapshot ->
            ...
            if (!response.isSuccessful) {
                throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_update_list_failed)))
            }
        }
    }
    ...
    suspend fun deletePersonalList(listId: String) {
        performOptimisticMutation(optimistic = { snapshot ->
            ...
            val response = traktAuthService.executeAuthorizedRequest { authHeader ->
                traktApi.deleteUserList(authorization = authHeader, id = ME_PATH, listId = listId)
            } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_request_failed))

            if (!response.isSuccessful && response.code() != 204) {
                throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_delete_list_failed)))
            }
        }
    }

    suspend fun reorderPersonalLists(orderedListIds: List<String>) {
        val rank = orderedListIds.mapNotNull { raw -> raw.removePrefix(PERSONAL_KEY_PREFIX).toLongOrNull() }
        ...
        performOptimisticMutation(optimistic = { snapshot ->
            ...
            rebuildSnapshot(remainingTabs + orderedTabs, snapshot.entriesByList)
        }) {
            val response = traktAuthService.executeAuthorizedRequest { authHeader ->
            ...
            if (!response.isSuccessful) {
                throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_reorder_lists_failed)))
            }
        }
    }
    ...
    private fun addItemToList(snapshot: Snapshot, item: LibraryEntryInput, listKey: String): Snapshot {
        ...
        val normalizedId = normalizeContentId(resolveIds(item), fallback = item.itemId.trim()).ifBlank { item.itemId.trim() }
    ...
    private fun rebuildSnapshot(
        ...
        rawEntriesByList.forEach { (listKey, entries) ->
            entries.forEach { entry ->
                membership.getOrPut(contentKey(entry.id, entry.type)) { mutableSetOf() }.add(listKey)
        ...
        val entriesByList = rawEntriesByList.mapValues { (_, entries) ->
            entries.map { entry -> entry.copy(listKeys = membership[contentKey(entry.id, entry.type)].orEmpty()) }
        }
    ...
    private suspend fun fetchSnapshot(): Snapshot {
        ...
        val rawEntriesByList = linkedMapOf<String, List<LibraryEntry>>().apply {
            ...
            personalTabs.forEach { tab -> put(tab.key, personalEntriesByList[tab.key].orEmpty()) }
        }
        ...
        rawEntriesByList.values.flatten()
            .sortedByDescending { it.listedAt }
            .forEach { entry -> allEntriesByContent[contentKey(entry.id, entry.type)] = entry }
    ...
    private suspend fun fetchWatchlistEntries(): List<LibraryEntry> {
        val movies = fetchAllPages<TraktListItemDto> { page ->
            traktAuthService.executeAuthorizedRequest { authHeader ->
                traktApi.getUserWatchlist(authorization = authHeader, id = ME_PATH, type = "movies", sort = "rank", page = page)
            } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_fetch_watchlist_movies))
        }
        val shows = fetchAllPages<TraktListItemDto> { page ->
            traktAuthService.executeAuthorizedRequest { authHeader ->
                traktApi.getUserWatchlist(authorization = authHeader, id = ME_PATH, type = "shows", sort = "rank", page = page)
            } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_fetch_watchlist_shows))
        }

        return (movies + shows)
            .mapNotNull { mapListItem(listKey = WATCHLIST_KEY, item = it) }
            .sortedWith(compareBy<LibraryEntry> { it.traktRank ?: Int.MAX_VALUE }.thenByDescending { it.listedAt })
    }
    ...
    private suspend fun fetchPersonalLists(): PersonalListFetchResult {
        ...
        } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_fetch_personal_lists))

        if (!response.isSuccessful) {
            throw IllegalStateException("${appContext.getString(R.string.trakt_library_error_fetch_personal_lists)} (${response.code()})")
        }

        val personal = response.body().orEmpty().filter { it.type.equals("personal", ignoreCase = true) }
                ...
                async {
                    semaphore.withPermit {
                        ...
                        if (listIdPath.isNullOrBlank()) {
                            ...
                            val movies = fetchPersonalListItems(listIdPath = listIdPath, type = "movie", listKey = tab.key, sortBy = tab.sortBy, sortHow = tab.sortHow)
                            val shows = fetchPersonalListItems(listIdPath = listIdPath, type = "show", listKey = tab.key, sortBy = tab.sortBy, sortHow = tab.sortHow)
                            tab.key to (movies + shows).sortedWith(compareBy<LibraryEntry> { it.traktRank ?: Int.MAX_VALUE }.thenByDescending { it.listedAt })
                        }
                    }
                }
            }.map { it.await() }.toMap()
    ...
    private suspend fun fetchPersonalListItems(
        ...
        val items = fetchAllPages<TraktListItemDto> { page ->
            ...
            } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_fetch_list_items))
        }
    ...
    private fun mapListTab(dto: TraktListSummaryDto): LibraryListTab? {
        ...
        return LibraryListTab(
            ...
            title = dto.name?.takeIf { it.isNotBlank() } ?: appContext.getString(R.string.library_list_fallback_title),
    ...
    private fun mapListItem(listKey: String, item: TraktListItemDto): LibraryEntry? {
        ...
        val mediaTitle = if (normalizedType == "movie") movie?.title else show?.title
        val mediaYear = if (normalizedType == "movie") movie?.year else show?.year
        val ids = if (normalizedType == "movie") movie?.ids else show?.ids
        val images = if (normalizedType == "movie") movie?.images else show?.images
        val overview = if (normalizedType == "movie") movie?.overview else show?.overview
        val releaseDate = if (normalizedType == "movie") movie?.released else show?.firstAired
        val rating = if (normalizedType == "movie") movie?.rating else show?.rating
        val genres = if (normalizedType == "movie") movie?.genres else show?.genres
    ...
    private suspend fun addToWatchlist(item: LibraryEntryInput) {
        ...
        if (!response.isSuccessful || !isSuccessfulAddResponse(response.body())) {
            throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_add_watchlist_failed)))
        }
    }
    ...
    private suspend fun removeFromWatchlist(item: LibraryEntryInput) {
        ...
        if (!response.isSuccessful) {
            throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_remove_watchlist_failed)))
        }
    }
    ...
    private suspend fun addToPersonalList(listId: String, item: LibraryEntryInput) {
        val body = buildMutationBody(item)
        val response = traktAuthService.executeAuthorizedRequest { authHeader ->
            traktApi.addUserListItems(authorization = authHeader, id = ME_PATH, listId = listId, body = body)
        } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_request_failed))

        if (!response.isSuccessful || !isSuccessfulAddResponse(response.body())) {
            throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_add_to_list_failed)))
        }
    }
    ...
    private suspend fun removeFromPersonalList(listId: String, item: LibraryEntryInput) {
        val body = buildMutationBody(item)
        val response = traktAuthService.executeAuthorizedRequest { authHeader ->
            traktApi.removeUserListItems(authorization = authHeader, id = ME_PATH, listId = listId, body = body)
        } ?: throw IllegalStateException(appContext.getString(R.string.trakt_library_error_request_failed))

        if (!response.isSuccessful) {
            throw IllegalStateException(errorMessageForCode(response.code(), appContext.getString(R.string.trakt_library_error_remove_from_list_failed)))
        }
    }
    ...
    private fun buildMutationBody(item: LibraryEntryInput): TraktListItemsMutationRequestDto {
        ...
        if (!ids.hasAnyId()) {
            throw IllegalStateException(appContext.getString(R.string.trakt_library_error_missing_compatible_ids))
        }
        val normalizedType = normalizeItemType(item.itemType)
    ...
    private fun errorMessageForCode(code: Int, defaultMessage: String): String = when (code) {
        401, 403 -> appContext.getString(R.string.trakt_library_error_auth_expired)
        404 -> appContext.getString(R.string.trakt_library_error_list_not_found)
        420 -> appContext.getString(R.string.trakt_library_error_list_limit_reached)
    ...
    private fun normalizeItemType(itemType: String): String = when (itemType.lowercase()) {
        "movie" -> "movie"
        "series", "show", "tv" -> "series"
        else -> itemType.lowercase()
    }

    private suspend fun <T> fetchAllPages(fetch: suspend (page: Int) -> Response<List<T>>): List<T> {
        val allItems = mutableListOf<T>()
        var currentPage = 1
        while (true) {
            val response = fetch(currentPage)
            if (!response.isSuccessful) {
                throw IllegalStateException(appContext.getString(R.string.trakt_library_error_paginated_fetch_failed, response.code()))
            }
