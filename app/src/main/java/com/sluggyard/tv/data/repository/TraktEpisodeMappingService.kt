...
/**
 * Reconciles addon episode numbering with Trakt's canonical episode numbering
 * for shows where the two differ (common with anime). Both directions are
 * cached and deduplicated in-flight so concurrent callers share a single
 * addon-meta and Trakt-seasons fetch per show.
 */
@Singleton
class TraktEpisodeMappingService @Inject constructor(
    ...
    internal suspend fun prefetchEpisodeMapping(
        ...
        season: Int?,
        episode: Int?
    ): EpisodeMappingEntry? = resolveEpisodeMapping(contentId, contentType, videoId, season, episode)

    /**
     * Pre-fetches addon episode data for [contentIds] so that subsequent calls
     * to [resolveAddonEpisodeMapping] hit the cache. Limits concurrency to
     * [concurrency].
     */
    internal suspend fun prefetchAddonEpisodes(
        ...
        supervisorScope {
            unique.forEach { contentId ->
                launch {
                    try {
                        semaphore.withPermit { getAddonEpisodes(contentId, "series") }
                    } catch (_: Exception) {
                        // Individual prefetch failures are non-fatal.
                    }
                }
            }
        }
    ...
    internal suspend fun resolveAddonEpisodeMapping(
        ...
        cacheMutex.withLock { reverseMappingCache[reverseKey]?.let { return it } }
        ...
        val addonHasEpisode = addonEpisodes.any { it.season == requestedSeason && it.episode == requestedEpisode }
        val sameStructure = hasSameSeasonStructure(addonEpisodes, traktEpisodes)
        if (addonHasEpisode && sameStructure) return null
        ...
        cacheMutex.withLock { reverseMappingCache[reverseKey] = mapped }
        return mapped
    }
    ...
    private fun hasSameSeasonStructure(
        ...
        // Anime often uses the same season numbers in both sources but with
        // completely different episode distributions.
    ...
    internal suspend fun resolveEpisodeMapping(
        ...
        val key = cacheKey(contentId, contentType, videoId, season, episode) ?: return null
        cacheMutex.withLock { mappingCache[key]?.let { return it } }
        ...
        if (hasSameSeasonStructure(addonEpisodes, traktEpisodes)) return null
        ...
        cacheMutex.withLock { mappingCache[key] = mapped }
        return mapped
    }

    private suspend fun getAddonEpisodes(contentId: String, contentType: String): List<EpisodeMappingEntry> {
        val cacheKey = addonEpisodesCacheKey(contentId, contentType)

        cacheMutex.withLock { addonEpisodesCache[cacheKey]?.let { return it } }
    ...
    private suspend fun getTraktEpisodes(showLookupId: String): List<EpisodeMappingEntry> {
        cacheMutex.withLock { traktEpisodesCache[showLookupId]?.let { return it } }
        ...
        val seasonsResponse = traktAuthService.executeAuthorizedRequest { authHeader ->
            traktApi.getShowSeasons(authorization = authHeader, id = showLookupId, extended = "episodes")
        } ?: run {
            cleanupTraktFlight(showLookupId)
            return emptyList()
        }
        if (!seasonsResponse.isSuccessful) {
            Log.w(TAG, "getTraktEpisodes: seasons request failed code=${seasonsResponse.code()} id=$showLookupId")
            cleanupTraktFlight(showLookupId)
            return emptyList()
        }

        val traktEpisodes = seasonsResponse.body().orEmpty().asSequence()
            ...
            .flatMap { seasonDto ->
                seasonDto.episodes.orEmpty().asSequence().mapNotNull { episodeDto ->
                    ...
                    EpisodeMappingEntry(season = seasonNumber, episode = episodeNumber, title = episodeDto.title)
                }
            }
            .toList()

        if (traktEpisodes.isNotEmpty()) {
            cacheMutex.withLock { traktEpisodesCache[showLookupId] = traktEpisodes }
        }
    ...
    private fun reverseCacheKey(contentId: String, contentType: String, season: Int, episode: Int, title: String?): String {
    ...
    private fun addonEpisodesCacheKey(contentId: String, contentType: String): String =
        "${contentType.trim().lowercase()}|${contentId.trim()}"
