...
class TorrServerBinary @Inject constructor(
    ...
    suspend fun start() = withContext(Dispatchers.IO) {
        ...
        if (!isBinaryAvailable) {
            throw TorrentException(context.getString(com.sluggyard.tv.R.string.torrent_error_binary_missing, binaryFile.absolutePath))
        }
        ...
        if (BuildConfig.DEBUG) Log.d(TAG, "Starting TorrServer on port $PORT from ${binaryFile.absolutePath}")
        process = pb.start()
        ...
        Thread {
            try {
                proc.inputStream.bufferedReader().forEachLine { line ->
                    if (BuildConfig.DEBUG) Log.d(TAG, "[server] $line")
                }
            } catch (_: Exception) {}
        }.apply {
            isDaemon = true
        ...
        while (System.currentTimeMillis() < deadline) {
            if (isRunning()) {
                if (BuildConfig.DEBUG) Log.d(TAG, "TorrServer started successfully")
                return@withContext
            }
            if (!isProcessAlive(process)) {
                val exitCode = process?.exitValue() ?: -1
                process = null
                throw TorrentException(context.getString(com.sluggyard.tv.R.string.torrent_error_process_died, exitCode))
            }
            delay(HEALTH_CHECK_INTERVAL_MS)
        }

        stop()
        throw TorrentException(context.getString(com.sluggyard.tv.R.string.torrent_error_start_timeout, (STARTUP_TIMEOUT_MS / 1000).toInt()))
    }
    ...
    private fun killOrphanedProcess() {
        try {
            ...
            Thread.sleep(1000)
            if (BuildConfig.DEBUG) Log.d(TAG, "Shut down orphaned TorrServer instance")
        } catch (_: Exception) {
    ...
    private fun terminateProcess(proc: Process) {
        proc.destroy()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && isProcessAlive(proc)) {
