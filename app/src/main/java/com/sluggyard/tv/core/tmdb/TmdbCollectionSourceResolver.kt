...
import com.sluggyard.tv.data.remote.api.TmdbCollectionSearchResult
import com.sluggyard.tv.data.remote.api.TmdbCompanySearchResult
...
import com.sluggyard.tv.domain.model.TmdbCollectionSort
...
class TmdbCollectionSourceResolver @Inject constructor(
    ...
    fun resolve(source: TmdbCollectionSource, page: Int = 1): Flow<NetworkResult<CatalogRow>> = flow {
        if (BuildConfig.TMDB_API_KEY.isBlank()) {
            emit(NetworkResult.Error("TMDB is not configured for this build."))
            return@flow
        }
